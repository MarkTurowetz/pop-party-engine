// Dual-use (server require + window global) controller layout widget art-composition map.
// Built to shared/controller-layout-art-widgets.js via `npm run build:shared`
...
(function (): void {
  ...
  function normalizeControllerLayoutWidgetId(value: unknown): string {
    return String(value || "")
      .trim()
      .replace(/^#/, "")
      .toLowerCase()
  ...
  function controllerLayoutWidgetArtCompositionId(elementIdOrSelector: unknown): string {
  ...
  if (typeof window !== "undefined") (window as unknown as Record<string, unknown>).PartyControllerLayoutArtWidgets = api;
})();
