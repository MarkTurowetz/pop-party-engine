...
(function (): void {
  ...
  const lobbyWidgetConfigs: LobbyWidgetConfig[] = [
    {
  ...
  function configForParent(compositionId: unknown): LobbyWidgetConfig | null {
    return lobbyWidgetConfigs.find((config) => config.parentId === compositionId) || null;
  }

  function configForChild(compositionId: unknown): LobbyWidgetConfig | null {
    return lobbyWidgetConfigs.find((config) => config.childId === compositionId) || null;
  }

  function lifecycleTimeline(referenceId: string): Record<string, unknown> {
    return {
      fps: 30,
      frameCount: 33,
      labels: [
  ...
  function baseTimeline(): Record<string, unknown> {
    return {
      fps: 30,
      frameCount: 1,
  ...
  function referenceComponent(config: LobbyWidgetConfig, canvas: Record<string, any>): Record<string, unknown> {
  ...
  function applyComponentLabels(config: LobbyWidgetConfig, components: Record<string, any>[]): void {
    for (const component of components) {
      if (config.componentLabels[component?.id]) component.instanceLabel = config.componentLabels[component.id];
    }
  }

  function installDefaultLobbyWidgetCompositions(compositions: Record<string, any>[]): Record<string, any>[] {
  ...
  function migrateLobbyWidgetComponents(compositionId: unknown, components: Record<string, any>[] = []): Record<string, any>[] {
    ...
    if (childConfig) applyComponentLabels(childConfig, components);
    return components;
  }

  function migrateLobbyWidgetReferenceBounds(
  ...
  function migrateLobbyWidgetKind(compositionId: unknown, compositionKind: unknown): string {
    return configForParent(compositionId) || configForChild(compositionId)
      ? "prefab"
      : String(compositionKind || "gameObject");
  }

  function migrateLobbyWidgetName(compositionId: unknown, name: unknown): string {
    const parentConfig = configForParent(compositionId);
    if (parentConfig && (!String(name || "").trim() || (parentConfig.parentId === "join-widget" && name === "Join Widget"))) {
      return parentConfig.parentName;
    }
    const childConfig = configForChild(compositionId);
    if (childConfig && !String(name || "").trim()) return childConfig.childName;
    return String(name || "");
  }

  function migrateLobbyWidgetTimeline(compositionId: unknown, timeline: any, fallbackTimeline: any): any {
    const config = configForParent(compositionId);
    if (!config) return timeline;
    const tracks = Array.isArray(timeline?.tracks) ? timeline.tracks : [];
    const hasReferenceTrack = tracks.some((track: any) => track?.targetId === config.referenceId);
    return hasReferenceTrack ? timeline : fallbackTimeline;
  }

  function legacyLobbyWidgetChildOverride(
    childCompositionId: unknown,
    manifestCompositions: Record<string, any> = {}
  ): Record<string, any> | null {
    const config = configForChild(childCompositionId);
    if (!config || manifestCompositions?.[config.childId]) return null;
    ...
    const legacyComponents = (Array.isArray(parent?.components) ? parent.components : [])
      .filter((component: any) => !(component?.kind === "reference" && component?.artCompositionId === config.childId));
    if (!legacyComponents.length) return null;
    return {
      name: config.childName,
  ...
  function lobbyWidgetChildIdForParent(parentCompositionId: unknown): string {
