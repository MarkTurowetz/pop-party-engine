...
function cleanRotationDirection(value: unknown): TimelineKeyframe["rotationDirection"] | undefined {
  const direction = cleanText(value, "", 40).toLowerCase();
  return direction === "clockwise" || direction === "counterclockwise" ? direction : undefined;
}

function cleanRotationTurns(value: unknown): number | undefined {
    ...
    .map((track) => {
      ...
      const keyframes = (Array.isArray(entry.keyframes) ? entry.keyframes : [])
        ...
        .map((keyframe) => {
          ...
          if (rotationDirection) normalizedKeyframe.rotationDirection = rotationDirection;
          if (rotationDirection && rotationTurns !== undefined) normalizedKeyframe.rotationTurns = rotationTurns;
...
function normalizeOffAnimationVisibility(timeline: TimelineDocument): TimelineDocument {
  ...
  const tracks = timeline.tracks.map((track) => {
    let keyframes = track.keyframes.map((keyframe) => {
      if (!offFrames.has(keyframe.frame)) return keyframe;
      const props: TimelineProperties = { ...keyframe.props, opacity: 1 };
      delete props.visible;
      return { ...keyframe, props };
    });
    const existingFrames = new Set(keyframes.map((keyframe) => keyframe.frame));
    for (const frame of offFrames) {
      if (existingFrames.has(frame)) continue;
      keyframes.push({
...
function defaultVisibilityCommandKey(command: TimelineCommand): string {
  return [command.frame, command.type, command.target || "", command.event || ""].join("|");
}

function defaultVisibilityLabelFrame(timeline: TimelineDocument, name: string): number {
  return timeline.labels.find((label) => label.name === name)?.frame ?? 0;
}

function mergeDefaultVisibilityKeyframeProps(
  track: TimelineTrack,
  frame: number,
  props: TimelineProperties,
  easing: string
...
export function timelineWithDefaultVisibility(
