...
try {
  ...
  fs.writeFileSync(path.join(fixtureRoot, "consumer.ts"), [
    ...
    'const plugin = defineGamePlugin({ namespace: "typed", register(registry) { registry.actions("typed.action", { name: "Typed Action", execute() {} }); registry.inputs("typed.choice", { name: "Typed Choice", fields: [{ key: "answersSubmittedTargetActionId", label: "After", control: "actionTarget" }], submission: [{ id: "choice", type: "choice", optionsSource: "options" }], controller: { layoutStateId: "typed-choice", bindings: [{ id: "left", kind: "choice", layoutElementId: "left", field: "choice", optionIndex: 0, autoSubmit: true }, { id: "targets", kind: "choiceCollection", layoutElementId: "target-options", field: "choice", item: { artCompositionId: "typed-controller-choice", targetComponentId: "option-label", labelSource: "label", disabledSource: "disabled" }, autoSubmit: true, holdSubmit: { seconds: 1.5, submitValues: {}, progress: { delaySeconds: 0.5, targetComponentId: "hold-meter", startLabel: "HoldStart", completeLabel: "HoldComplete", resetLabel: "Off" } } }] }, recipients(context) { return context.players.map((player) => player.id); }, view(context) { return { viewer: context.viewer.id, options: [{ id: "left", label: "Left", disabled: false }] }; }, submit(context, payload) { context.state.choice = payload.choice; } }); registry.stageRenderers("typed.players", { name: "Player presentation", target: { kind: "layout", layoutElementId: "players", layoutScope: "global" }, bindings: [{ id: "players", kind: "collection", source: "players", item: { keySource: "id", artCompositionId: "typed-player", bindings: [{ id: "score", kind: "text", source: "score", targetComponentId: "score" }] } }], select(context) { return { players: context.players.map((player) => ({ id: player.id, score: "0" })) }; } }); } });',
