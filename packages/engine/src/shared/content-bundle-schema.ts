...
(function (root: Record<string, unknown>, factory: () => unknown): void {
  ...
  else root.PartyGameContentBundleSchema = api;
  ...
  function canonicalizeJson(value: unknown): string {
    function canonical(valueAtPath: unknown, logicalPath: string): string {
      if (valueAtPath === null) return "null";
      if (typeof valueAtPath === "string" || typeof valueAtPath === "boolean") return JSON.stringify(valueAtPath);
      if (typeof valueAtPath === "number") {
        if (!Number.isFinite(valueAtPath)) throw new Error(`Non-finite number at ${logicalPath}`);
      ...
      if (typeof valueAtPath === "object") {
        const record = valueAtPath as Record<string, unknown>;
        const keys = Object.keys(record).sort();
        return `{${keys.map((key) => {
          if (record[key] === undefined) throw new Error(`Undefined value at ${logicalPath}.${key}`);
  ...
  function normalizeManifest(value: unknown): ContentBundleManifest {
    if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Content bundle manifest must be an object");
    ...
    if (!GAME_ID_PATTERN.test(gameId)) throw new Error(`Invalid content bundle game id: ${gameId}`);
    ...
    const files = fileInputs.map((fileValue, index): BundleFileRecord => {
      ...
      if (filePath === CONTENT_BUNDLE_MANIFEST_PATH) throw new Error("The bundle manifest cannot hash itself");
      const portableKey = filePath.normalize("NFC").toLocaleLowerCase("en-US");
      if (seenExact.has(filePath) || seenPortable.has(portableKey)) throw new Error(`Duplicate or colliding bundle path: ${filePath}`);
      seenExact.add(filePath);
      seenPortable.add(portableKey);
      const fileSha = String(record.sha256 || "").toLowerCase();
      if (!SHA256_PATTERN.test(fileSha)) throw new Error(`Invalid SHA-256 for ${filePath}`);
      const bytes = Number(record.bytes);
      if (!Number.isInteger(bytes) || bytes < 0) throw new Error(`Invalid byte size for ${filePath}`);
      return Object.freeze({ path: filePath, sha256: fileSha, bytes });
    }).sort((left, right) => left.path.localeCompare(right.path));
    for (const requiredPath of REQUIRED_CONTENT_PATHS) {
      if (!seenExact.has(requiredPath)) throw new Error(`Content bundle is missing required file: ${requiredPath}`);
    }
    if (!seenExact.has(semanticRolesPath)) throw new Error(`semanticRolesPath is not present in files: ${semanticRolesPath}`);
    const rootHash = String(input.rootHash || "").toLowerCase();
    if (!SHA256_PATTERN.test(rootHash)) throw new Error("Content bundle rootHash must be SHA-256");
  ...
  function rootHashInput(files: BundleFileRecord[]): string {
    return [...files]
      .sort((left, right) => left.path.localeCompare(right.path))
