...
(function (): void {
  ...
  interface ControllerButtonConfig {
    parentId: string;
    parentName: string;
    interactionId: string;
    interactionName: string;
    interactionReferenceId: string;
    interactionInstanceLabel: string;
    stateId: string;
    stateName: string;
    stateReferenceId: string;
    stateInstanceLabel: string;
    artId: string;
    artName: string;
    artReferenceId: string;
    artInstanceLabel: string;
  }

  const controllerButtonConfigs: ControllerButtonConfig[] = [
  ...
  function buttonConfig(parentId: string, parentName: string): ControllerButtonConfig {
  ...
  function applyBaseInstanceLabels(components: Record<string, any>[]): Record<string, any>[] {
  ...
  function configForParent(compositionId: unknown): ControllerButtonConfig | null {
    return controllerButtonConfigs.find((config) => config.parentId === compositionId) || null;
  }

  function configForArt(compositionId: unknown): ControllerButtonConfig | null {
  ...
  function stateTimeline(artReferenceId: string): Record<string, unknown> {
    return {
      fps: 30,
      frameCount: 2,
  ...
  function interactionTimeline(stateReferenceId: string, canvas: Record<string, any>): Record<string, unknown> {
  ...
  function lifecycleTimeline(interactionReferenceId: string): Record<string, unknown> {
    return {
      fps: 30,
      frameCount: 19,
      labels: [
  ...
  function installDefaultControllerButtonCompositions(compositions: Record<string, any>[]): Record<string, any>[] {
    for (const config of controllerButtonConfigs) {
      ...
      parent.components = [referenceComponent(
        config.interactionReferenceId,
        config.interactionName,
        config.interactionInstanceLabel,
  ...
  function isNestedParentOverride(config: ControllerButtonConfig, override: Record<string, any> | null): boolean {
    return Boolean((Array.isArray(override?.components) ? override.components : []).some(
      (component: any) => component?.kind === "reference" && component?.artCompositionId === config.interactionId
    ));
  }

  function controllerButtonOverride(
  ...
  function controllerButtonInteractionReferenceId(parentCompositionId: unknown): string {
    return configForParent(parentCompositionId)?.interactionReferenceId || "";
  }

  function controllerButtonStateReferenceId(parentCompositionId: unknown): string {
